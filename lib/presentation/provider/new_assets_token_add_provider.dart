...
class NewAssetsTokenAddProvider extends ChangeNotifier {
  ...
  Future showAddedTokenAndBalance() async {
    ...
    allScreenTokenList[0]['value'] = double.parse(data['data']['market'][3]['new_price']).toStringAsFixed(2);
    allScreenTokenList[1]['value'] = double.parse(data['data']['market'][3]['new_price']).toStringAsFixed(2);
    allScreenTokenList[2]['value'] = double.parse(data['data']['market'][4]['new_price']).toStringAsFixed(2);
    allScreenTokenList[3]['value'] = double.parse(data['data']['market'][1]['new_price']).toStringAsFixed(2);
    notifyListeners();
  }

  enableDisableMaker(bool enable, int index){
    allScreenTokenList[index]['show'] = enable;
    notifyListeners();
  }
